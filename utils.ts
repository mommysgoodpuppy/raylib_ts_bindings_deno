export const typeMapping: Record<
  string,
  {
    ffi: Deno.NativeType | Deno.NativeVoidType | Deno.NativeStructType;
    deno: string;
    c: string;
  }
> = {
...
export function readBufferStructured(
  view: DataView,
  template: any,
  offset = 0,
): [any, number] {
    ...
    if (typeof value === "boolean") {
      ...
      for (const [subKey, subValue] of Object.entries(nestedValue)) {
        if (subKey === "m" || subKey === "v") {
          [result[key][subKey], offset] = subKey === "m"
            ? readMatrix(view, subValue as number[][], offset)
            : readVector(view, subValue as number[], offset);
        } else {
          [result[key][subKey], offset] = readBufferStructured(view, {
            [subKey]: subValue,
          }, offset);
